...
from lie_conv.lieGroups import SO2, T, Trivial, Reflections
...
def test_gnn_conv_equivariance(groups_and_trans: tuple = ((T(2), shift_x), (Reflections(2), reflect_all)), bs: int = 2,
                               device='cpu', h: int = 10, w: int = 10, liftsamples: int = 1, n_mc_samples: int = 25,
                               ds_frac: float = 1., fill: float = 1., conv_layer=LieConvGCN):
    device = torch.device(device)
    ...
    for group, trans in groups_and_trans:
        ...
        assert (conv_vals[0] == conv_vals[1]).all(), \
            f'Error - layer is not equivariant! Group - {group}'
